...
const EditLive = () => {
  ...
  if(location?.state?.date){
    chat = location.state.date
  } else{
    chat = location.state.data[0]
  }
 console.log(chat)
//   const chat = location?.state?.date;
  const messages = chat?.messages || [];
    console.log('all messages', chat[0]?.messages)
  return (
    <div className="p-6 bg-[#fefefe] min-h-screen font-mono text-sm">
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <Button onClick={()=> dispatch(addEdited(chat))} icon={<CopyOutlined />}>Save</Button>
